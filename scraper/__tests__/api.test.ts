...
describe('parseInfoMacros', () => {
  ...
  it('handles decimals with dot or comma', () => {
    expect(parseInfoMacros('606 kcal • B:18g • W:61g • T:32g')).toMatchObject({ kcal: 606, protein_g: 18 });
    expect(parseInfoMacros('300.5 kcal • B:19,5g')).toMatchObject({ kcal: 300.5, protein_g: 19.5 });
  });

  it('returns nulls for missing fields', () => {
    expect(parseInfoMacros('300 kcal')).toEqual({ kcal: 300, protein_g: null, carbs_g: null, fat_g: null });
    expect(parseInfoMacros(null)).toEqual({ kcal: null, protein_g: null, carbs_g: null, fat_g: null });
    expect(parseInfoMacros('')).toEqual({ kcal: null, protein_g: null, carbs_g: null, fat_g: null });
  });
