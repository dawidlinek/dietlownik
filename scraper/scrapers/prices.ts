...
export async function scrapePrices(companyId: string, cityId: number): Promise<void> {
  ...
  for (const leaf of leaves) {
    const includeSaturday = leaf.delivery_on_saturday ?? false;

    for (const days of ORDER_DAY_TIERS) {
      const deliveryDates = futureWeekdays(days, { includeSaturday });
      ...
      try {
        ...
        console.warn(`[prices] skip ${companyId} cal=${leaf.diet_calories_id} days=${days}: ${(err as Error).message}`);
        continue;
      }
  ...
  console.log(`[prices] ✓ ${companyId}: ${inserted} price rows inserted`);
}
