...
const checks: (() => Promise<Finding>)[] = [
  ...
  async () => {
    const rows = await query<{ t: string; n: string }>(
      `SELECT 'companies' AS t, COUNT(*)::text AS n FROM companies
       ...
       UNION ALL SELECT 'daily_menu',      COUNT(*)::text FROM daily_menu
       UNION ALL SELECT 'meal_embeddings', COUNT(*)::text FROM meal_embeddings
  ...
  async () => {
    const [r] = await query<{
      ...
      `SELECT
         COUNT(*)::text AS total,
         COUNT(*) FILTER (WHERE ingredients_raw IS NULL OR ingredients_raw = '')::text AS null_ingredients,
         COUNT(*) FILTER (WHERE allergens IS NULL OR cardinality(allergens) = 0)::text AS null_allergens,
    ...
    if (ni / total > 0.05 || nm / total > 0.05 || na / total > 0.3) {
      sev = "WARN";
    }
    return {
      detail: `ingredients_NULL=${pct(ni)}  allergens_NULL=${pct(na)}  kcal_NULL=${pct(nk)}  macros_NULL=${pct(nm)}  (of ${fmt(total)})`,
      ...
      `SELECT
         ...
         (SELECT COUNT(*) FROM companies c
            WHERE EXISTS (SELECT 1 FROM diets d WHERE d.company_id = c.company_id)
              AND NOT EXISTS (SELECT 1 FROM daily_menu dm WHERE dm.company_id = c.company_id))::text AS no_menus_with_diets,
         (SELECT COUNT(*) FROM daily_menu WHERE meal_id IS NULL)::text AS menu_no_meal`
    );
    const e = Number.parseInt(r.empty_companies, 10);
    const n = Number.parseInt(r.no_menus_with_diets, 10);
    const m = Number.parseInt(r.menu_no_meal, 10);
    const sev = e > 0 || n > 0 ? "WARN" : "OK";
    return {
      detail: `companies-no-meals=${e}  diets-without-menus=${n}  menu-slots-without-meal=${m}`,
  ...
  async () => {
    const [r] = await query<{
      ...
      `SELECT
         (SELECT COUNT(*) FROM meals)::text AS total,
         (SELECT COUNT(*) FROM current_meal_embeddings)::text AS embedded,
         (SELECT COUNT(*) FROM meals m
            JOIN current_meal_embeddings e ON e.meal_id = m.id
           WHERE e.embedded_fp <> COALESCE(m.fingerprint, ''))::text AS stale`
    );
    const total = Number.parseInt(r.total, 10);
  ...
  async () => {
    const [r] = await query<{
      ...
      `SELECT menu_date::text, COUNT(DISTINCT meal_id)::text AS meals, COUNT(DISTINCT company_id)::text AS companies
         FROM daily_menu
        WHERE city_id = 986283 AND meal_id IS NOT NULL
        GROUP BY menu_date
  ...
  async () => {
    const [r] = await query<{
      ...
      `SELECT
         (SELECT COUNT(*) FROM meals WHERE kcal > 5000)::text AS huge_kcal,
         (SELECT COUNT(*) FROM meals WHERE protein_g < 0 OR fat_g < 0 OR carbs_g < 0 OR kcal < 0)::text AS negative_macros,
         (SELECT COUNT(*) FROM meals WHERE kcal IS NOT NULL AND kcal < 10 AND kcal > 0)::text AS tiny_kcal`
    );
  ...
  // ── Fingerprint duplicates (informational — same recipe at multiple caterers) ─
  async () => {
    const [r] = await query<{ dup: string; pct: string }>(
      `SELECT (COUNT(*) - COUNT(DISTINCT fingerprint))::text AS dup,
              ROUND(100.0 * (COUNT(*) - COUNT(DISTINCT fingerprint)) / NULLIF(COUNT(*),0), 1)::text AS pct
         FROM meals
    ...
    return {
      detail: `${fmt(d)} meals share fingerprint with another (${p.toFixed(1)}%) — likely same recipe across companies`,
