...
async function smokeTestFromBun(cookie: string, userAgent: string): Promise<{ status: number; body: string }> {
  ...
  try {
    ...
    for (const url of WARM_URLS) {
      ...
      try {
        ...
        log(`  (nav: ${(err as Error).message.split('\n')[0]} — continuing)`);
      }
      const cleared = await waitForChallengeCleared(page, deadlineAt);
      if (!cleared) log('  (CF challenge did not clear within budget — continuing)');
      // CF / site JS often sets cookies after DCL.
      await page.waitForTimeout(1500);
    }

    const cookies = (await ctx.cookies()).filter(c => c.domain.endsWith('dietly.pl'));
    if (cookies.length === 0) {
      throw new Error('no *.dietly.pl cookies captured — chrome session is empty');
    }
    const cookieStr = cookies.map(c => `${c.name}=${c.value}`).join('; ');
    const userAgent = await page.evaluate(() => navigator.userAgent);
    log(`captured ${cookies.length} cookies: ${cookies.map(c => c.name).join(', ')}`);
    ...
    if (smoke.status === 403 && CF_CHALLENGE_RE.test(smoke.body)) {
      throw new Error('bun fetch was CF-challenged — captured cookies do not satisfy CF for this client');
    }
    if (smoke.status >= 500) {
    ...
    if (smoke.status !== 200) {
      log(`  note: smoke test returned ${smoke.status} (CF passed; app body: ${smoke.body.slice(0, 120)})`);
    }
  } finally {
    if (KEEP_OPEN) log('CF_KEEP_OPEN=1 — leaving browser open; ctrl-c to exit');
