...
describe.skipIf(HEAVY_SKIP)("bge-m3 vector mechanics", () => {
  ...
  it("preserves per-item identity across multi-element batches", async () => {
    ...
    // Batch and per-item runs sometimes diverge by tiny float-quantisation
    // noise. Cosine should still be effectively 1.0.
    expect(cosine(a, aSolo)).toBeGreaterThan(0.999);
    expect(cosine(b, bSolo)).toBeGreaterThan(0.999);
    expect(cosine(c, cSolo)).toBeGreaterThan(0.999);
  });
...
describe.skipIf(HEAVY_SKIP)("bge-m3 semantic ordering on Polish food terms", () => {
  vi.setConfig({ testTimeout: 300_000 });
  ...
  it("connects allergen names to their staple sources", async () => {
    const { getEmbedder, cosine } = await import("../embeddings.js");
    const embedder = await getEmbedder();
    const [gluten, wheat, milk, apple] = await embedder.embedBatch([
      "gluten",
      "pszenica",
      "mleko",
      "jabłko",
    ...
    expect(cosine(milk, gluten)).toBeLessThan(cosine(milk, wheat));
  });
  ...
  it("preserves diacritic-sensitivity reasonably (białko ≈ bialko)", async () => {
    ...
    expect(cosine(precise, ascii)).toBeGreaterThan(cosine(precise, unrelated));
  });
});
